...
/*	fprintf(cgiOut, "<head><meta charset=\"utf-8\"/><title>查询结果</title>\
	...
	if (sname[0] == '*')
	{
		sprintf(sql, "select sno,sname,sage,ssex,mno from info where state != '0'");
	}
	else
	{
		sprintf(sql, "select sno,sname,sage,ssex,mno from info where sname = '%s' and state !='0'", sname);
	}
